...
class votacion_controller extends CI_Controller {
	...
	{
            if ($this->session->userdata('logueado') == TRUE) {
            $this->votacion_model->eliminar($id);
            redirect('/votacion_controller/index','refresh');
            }else{
                  redirect('loginController/index','refresh');
            }	
	}
...
/*	public function eliminar($id)
	{
		...
		if ($msj == "success") 
		{
			...
			$datos['estado'] = $this->votacion_model->get_estado();			
      		...
      		$this->load->view('votacion_view',$datos);
      	}
      	else{
      		$datos['votacion'] = $this->votacion_model->get_votacion();
      		$datos['estado'] = $this->votacion_model->get_estado();      		
      ...
      {
            if ($this->session->userdata('logueado') == TRUE) {
            $datos['votacion'] = $this->votacion_model->get_datos($id);
