...
class DV_model extends CI_Model {
	...
	public function get_DV()
	{
		...
		$this->db->join('partido_politico pp','pp.id_partido = c.id_partido','left');
	...
	{
		$this->db->select('c.id_candidato, c.id_votacion, pc.nombre1, pc.nombre2, pc.apellido1, pc.apellido2, c.id_partido, pp.nombre_partido'); 
		$this->db->from('candidatos c');
		$this->db->join('persona pc','pc.DUI_persona = c.DUI_persona');
		$this->db->join('partido_politico pp','pp.id_partido = c.id_partido');
		$exe = $this->db->get();
